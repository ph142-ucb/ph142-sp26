...
Reference: [Philip Bump. "How to mislead with statistics, DHS Secretary Nielsen edition". Washington Post. June 18th, 2018.](https://www.washingtonpost.com/news/politics/wp/2018/06/18/how-to-mislead-with-statistics-dhs-secretary-nielsen-edition/)
...
This great video summarizes the issue nicely:
...
From [Baldi & Moore, Example 9.2 on page 448](https://ebookcentral-proquest-com.libproxy.berkeley.edu/lib/berkeley-ebooks/reader.action?docID=6643934&ppg=448):
...
```{r first-sample-250, echo=F,fig.align='center', message=F, warning=F, out.width="60%"}
# students, you don't need to understand the code in this chunk
...
```{r sample-250-500, echo=F, fig.align='center', message=F, warning=F, out.width="60%"}
...
```{r sample-5000, echo=F, fig.align='center', message=F, warning=F, out.width="60%"}
...
event occurring. This "not occurring" event is called the **complement**. 
    ...
    * Another way to write "A not occurring" is using $A'$ (pronounced A prime or A naught) or $A^c$: $P(A') = 1 - P(A)$ or $P(A^c) = 1 - P(A)$
...
```{r cs-variation-plot-example, echo = F, warning=F, message=F, out.width="60%"}
...
- [From the CDC](https://www.cdc.gov/nchs/data/dvs/LCWK9_2015.pdf) we have the probability of dying from a specific cause 
