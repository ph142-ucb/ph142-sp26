...
This is what is known as the **large sample confidence interval for a population proportion**, or the Wald interval.
...
- This confidence interval can perform poorly, meaning that if we repeated the confidence interval 100 times (based on 100 random samples), the coverage could be what is termed 
  ...
  - For the Wald interval, for different combinations of $p$ and $n$, it can exhibit **both** problems!  
...
- In addition, for smaller samples (when one cannot rely on the CLT), one can rely on alternative methods to get inference that do not rely on Normality of $\hat{p}$
...
- Note: if the number of successes and failures is relatively large, then the "plus four" method will converge with the large sample Wald CI method
...
- Using the large sample Wald method, the 95% confidence interval is 16.5% to 23.5% 
...
- Note that this interval is wider than the one made with the large sample Wald method
...
|:------------:|:-----------------------:|:----------:|
| Large sample Wald | 16.5% to 23.5%          | by hand      |
...
- Only the large sample Wald method is symmetric around $\hat{p} = 20\%$. This is
...
| Method       | 95% Confidence Interval | R Function       |
|--------------|:-----------------------:|--------------|
| Large sample Wald | -0.74% to 4.74%          | by hand      |
...
library(ggplot2)
library(tibble)
elderly_CIs <- tibble(method = c("large sample Wald", "Exact", "Wilson", "Plus 4"),
...
- Notice how different the intervals are, especially large sample Wald vs. others
- Notice that the large sample Wald lower bound is nonsensical (i.e., we can't have negative proportions!)
- The large sample Wald CI differs from the others because the Normal approximation assumptions are not satisfied
...
- However, there are problems with the coverage of the CIs:
  ...
  - The coverage problem goes away as the sample size gets larger, and more of the Normality assumptions are met