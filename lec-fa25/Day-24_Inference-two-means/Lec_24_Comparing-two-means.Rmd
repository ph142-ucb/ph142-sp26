...
deviations of the populations are unknown. 
...
the weighted sum of their respective variances. That is:
...
But what about $\mu_1$ and $\mu_2$? Where do we get them?

### The degrees of freedom (df) for the two-sample $t$-test...

is bananas. Here is the formula:

$$df = \frac{\Big(\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}\Big)^2}{\frac{1}{n_1-1}\Big(\frac{s_1^2}{n_1}\Big)^2 + \frac{1}{n_2-1}\Big(\frac{s_2^2}{n_2}\Big)^2}$$
...
Here $t^*$ is the critical value with area C between $-t^*$ and $t^*$ under the $t$-distribution with the appropriate degrees of freedom.
...
Under the two-sided null hypothesis:
...
- To answer this, we would need to calculate the degrees of freedom using the complicated formula from a previous slide
- We won't do this by hand
- Instead, we will ask R to do the test for us 
...
```{r}
commercial_weight <- chicken_data %>% 
  filter(type == "commercial") %>% pull(weight)
transgenic_weight <- chicken_data %>% 
  filter(type == "transgenic") %>% pull(weight)
```
...
:::: column
```{r}
t.test(commercial_weight, transgenic_weight, 
       alternative = "two.sided")
```
...
`r #  - Though still requires underlying random variable is Normal within the two groups if sample size is relatively small in either group`
