...
author: "Instructors: Tomer Altman"
date: "October 1, 2025"
...
p1 <- ggplot(data = data.frame(x = c(30,60)), aes(x)) +
    ...
    labs(y = "f(x)", x = "x") + 
    annotate("text", x = 35,y = 0.090, label = "N(45,4) density", check_overlap = T, col = "black") +
...
p2 <- ggplot(data = data.frame(x = c(30,60)), aes(x)) +
  ...
  labs(y = "P(X < x)", x = "x") + 
  annotate("text", x = 37,y = 0.9, label = "N(45,4) Cumulative Dist", check_overlap = T, col = "black") +
  theme_minimal(base_size = 15) +
  scale_x_continuous(breaks = seq(30, 60, by = 4)) +
...
That is, what is the value of $x$ such that 75% of the probability is under the curve "to the left" of $x$?
::::
...
ggplot(data = data.frame(x = c(29, 61)), aes(x)) +
  ...
  labs(y = "density") +
  annotate("text", x = 44, y = 0.05, check_overlap = T, label = "75%", col = "white", size = 8) 
```
::::
:::
...
still sometimes hard to say if it is truly Normal.

- Think of the Q-Q plot as a magnifying lens for visualizing whether a curve is truly Normal
