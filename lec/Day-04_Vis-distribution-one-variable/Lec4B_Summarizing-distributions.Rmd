---
title: "Describing Distributions with Numbers (Chapter 2)"
...
library(ggplot2)
library(dplyr)
library(fGarch) # students, you don't need to know what this library does. (It's used to make the skewed distributions)
```
...
sym_plot <- ggplot(rent_data, aes(x = sym)) +
  ...
  annotate('text', x = rent_summs$sym_mean + 150, y = 20,
            label = "Mean", check_overlap = T) +
  annotate('text', x = rent_summs$sym_median - 150, y = 20,
...
* [Data visualization aside: For those wondering how to bedazzle your plots, here is an easier way to pick colors for plotting: [Hex color picker](https://www.w3schools.com/colors/colors_picker.asp) ]
...
sym_out_right_plot <- ggplot(rent_data, aes(x = sym_out_right)) +
  ...
  annotate('text', x = rent_summs$sym_out_right_mean + 350, y = 20,
            label = "Mean", check_overlap = T) +
  annotate('text', x = rent_summs$sym_out_right_median - 350, y = 20,
            label = "Median", check_overlap = T, col = "blue") +
...
sym_out_right_plot <- ggplot(rent_data_small, aes(x = sym_out_right)) +
  ...
  annotate('text', x = rent_summs_small$sym_out_right_mean + 350, y = 20,
            label = "Mean", check_overlap = T) +
  annotate('text', x = rent_summs_small$sym_out_right_median - 350, y = 20,
            label = "Median", check_overlap = T, col = "blue") +
  geom_rug(col = "#005824", alpha = 0.5) +
  theme_minimal(base_size = 15)
...
bi_plot <- ggplot(rent_data, aes(x = bimodal)) +
  ...
  annotate('text', x = rent_summs$bimodal_mean + 150, y = 20,
            label = "Mean", check_overlap = T) +
  annotate('text', x = rent_summs$bimodal_median - 150, y = 20,
            label = "Median", check_overlap = T, col = "blue") +
  geom_rug(col = "#005824", alpha = 0.5) +
  theme_minimal(base_size = 15)
bi_plot
...
right_skew_plot <- ggplot(rent_data, aes(x = right_skew)) +
  ...
  annotate('text', x = rent_summs$right_skew_mean + 150, y = 20, label = "Mean", check_overlap = T) +
  annotate('text', x = rent_summs$right_skew_median - 150, y = 20, label = "Median", check_overlap = T, col = "blue") +
  geom_rug(col = "#005824", alpha = 0.5) +
  theme_minimal(base_size = 15)
right_skew_plot
```
...
These data were provided by the first author (Kozhimannil) of an [article](https://www.healthaffairs.org/doi/10.1377/hlthaff.2012.1030) 
...
being the ordered values above (but not including) the median. Take the median for
...
* [This video](https://www.khanacademy.org/math/ap-statistics/summarizing-quantitative-data-ap/more-standard-deviation/v/review-and-intuition-why-we-divide-by-n-1-for-the-unbiased-sample-variance) provides intuition of why the sample variance is smaller than the population variance
* To understand why we divide specifically by $n-1$, see [Bessel's Correction](https://en.wikipedia.org/wiki/Bessel%27s_correction#Source_of_bias) (for your own curiosity; won't be tested on this)
...
$s^2 = \frac{1}{n-1}\left[(x_1-\bar{x})^2 + (x_2-\bar{x})^2 + ... + (x_n-\bar{x})^2\right]$
