...
* See how the quality of a probability estimate is related to the size of the sample chosen
...
- The most common type of probability sample is a **simple random sample** (SRS)
...
`CA_data %>% group_by(county) %>% slice_sample(prop = 0.1)`
...
```{r ,out.width="80%"}
CS_data %>% group_by(HOSP_BEDSIZE) %>% summarize(counts = n())
```
::::
...
( `summarize(counts = n())` is almost the same as `count()` )
...
within schools vs. an SRS of students across a set of schools. How would these 
...
estimate some quantities for all the schools?
...
- You can find more information about the data set in the [User Guide](https://data.nber.org/nvss/natality/inputs/pdf/2016/UserGuide2016.pdf)
