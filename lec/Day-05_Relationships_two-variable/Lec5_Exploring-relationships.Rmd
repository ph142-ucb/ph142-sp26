...
- Please *only* use the `142gsi@berkeley.edu` email address for contacting the instructors
...
```{r bivariate-data, echo=F, message=F}
...
- To know more, take a class specifically about clinical trial design or take introduction to epidemiology to learn all about conducting observational studies
...
```{r scatter-plot-enzyme, out.width="50%"}
...
#Fill in during class
weight_scatter_by_gender <- ggplot(weight_data, aes(x=mass, y=rate)) +
  geom_point() +
  theme_minimal(base_size = 15) +
  facet_wrap(~gender)

weight_scatter_by_gender
```
...
```{r corr.plots, out.width="60%", echo=F, comment=NA, message=F}
...
corr.plot <- ggplot(corr_data, aes(x,y)) + 
  annotate('text', x = 5, y = 35, label = 1, size = 60, col = "cadetblue1") +
  annotate('text', x = 15, y = 35, label = 2, size = 60, col = "cadetblue1") +
  annotate('text', x = 5, y = 10, label = 3, size = 60, col = "cadetblue1") +
  annotate('text', x = 15, y = 10, label = 4, size = 60, col = "cadetblue1") +
  geom_point() + 
...
```{r load-dplyr, echo=F, warning = F, results = 'hide', message=F}
library(dplyr)
