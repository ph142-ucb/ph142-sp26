...
* [Chapter 13 of Baldi and Moore](https://ebookcentral-proquest-com.libproxy.berkeley.edu/lib/berkeley-ebooks/reader.action?docID=6643934&ppg=676&c=RVBVQg)
  - The sampling distribution of $\bar{x}$; page 689 of online textbook
  - The Central Limit Theorem; page 694 of online textbook
* [Learning Statistics with R](https://learningstatisticswithr.com/lsr-0.6.pdf)
  - Sampling distribution of the mean; see 10.3.1 (page 309)
  - The Central Limit Theorem; see 10.3.3 (page 325)
...
Note that the $p$ is also the mean of a binary (i.e., $0$ or $1$) random variable.
...
```{r, out.width="80%", echo=F, message=F, warning=F, fig.align='center'}
library(tidyverse)
...
sample mean $\bar{x}$ of a simple random sample of size $n$ has a $N\left(\mu, \frac{\sigma}{\sqrt{n}}\right)$
...
the $1,000$ means using a histogram
...
1) Take a sample from the distribution of size $1,000$
2) For your sample calculate the mean
3) Repeat steps $1$ and $2$ one-thousand times
4) Plot the sampling distribution of the mean. That is, plot the distribution of 
the 1000 means using a histogram.
...
- For this to apply, we require:
    ...
    - both $np$ and n(1-p) are equal to or larger than 10.
