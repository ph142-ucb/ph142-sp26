...
- All but one of the assumptions could not be checked using plots
...
```{r broom-functions-tidy}
frog_lm <- lm(formula = freq ~ temp, data = frog_data)
tidy(frog_lm)
```

### Use `lm()` + `broom`: `glance()`
```{r broom-functions-glance}
glance(frog_lm) 
```

### Use `lm()` + `broom`: `augment()`
```{r broom-functions-augment}
...
* We are interested in knowing the average spread of the squared residual distances
* Because small spread would indicate a good fitting model
* To measure this, we calculate the **regression standard error**
...
- `sigma` is shown as 2.82, which is equivalent to $s = 2.82$ 
- This is the residual estimated standard deviation (or sometimes called residual standard error)
...
- The IQR for the residuals goes from -1.92 to 1.00, or a width ($Q_3 - Q_1$) of 2.92
...
```{r, echo=F, fig.align='center', out.width="50%", message=F}
ggplot(frog_data, aes(x = temp, y = freq)) +
  theme_minimal(base_size = 15) +
  labs(x = "Temperature (Celsius)", y = "Frequency of mating calls") + 
  geom_hline(aes(yintercept = 47), col = "red") +
  annotate("text", x = 20, y = 48, label = "Null hypothesized slope", col = "red", check_overlap = T) +
  geom_smooth(method = "lm", se = F) +
  annotate("text", x = 20, y = 43, label = "Estimated slope", col = "blue", check_overlap = T) +
  geom_point()
```
...
- We will use R to compute the test statistic, $SE_b$ and $s$
- Be sure you know which functions to use to find $SE_b$, $s$, and $\hat{b}$
- Know where to look in the output to find them
...
:::: column
```{r, echo=F, fig.align='center', out.width="100%", message=F}
ggplot(frog_data, aes(x = temp, y = freq)) +
  theme_minimal(base_size = 15) +
  labs(x = "Temperature (Celsius)", y = "Frequency of mating calls") + 
  geom_smooth(method = "lm", se = F) +
  geom_vline(aes(xintercept = 21), col = "orange", lty = 2)+
  annotate("point", x = 21, y = 42.8, col = "orange") +
  geom_point()
```
::::
:::
...
$\hat{y} \pm t^* \times SE_{\hat{\mu}}$, where $SE_{\hat{\mu}}=s\sqrt{\frac{1}{n} + \frac{(x^*-\bar{x})^2}{\sum(x-\bar{x})^2}}$

Stated plainly: what is the confidence interval for the *average* frog's frequency being $\hat{y}$ when the temperature is $21 ^\circ C$?

If you want to make inference for a **single observation $y$** when $x$ takes the value
$x^*$ ($x^* = 21$ in our example):

$\hat{y} \pm t^* \times SE_{\hat{y}}$, where $SE_{\hat{y}}=s\sqrt{1 + \frac{1}{n} + \frac{(x^*-\bar{x})^2}{\sum(x-\bar{x})^2}}$

Stated plainly: what is the confidence interval for *one* frog's frequency being $\hat{y}$ when the temperature is $21 ^\circ C$?
...
plot_a <- ggplot(frog_data, aes(x = temp, y = freq)) +
  ...
  annotate("segment", x = 21, xend = 21, y = 40.38472, yend = 45.12887, col = "red") + # confidence interval
  geom_point() + 
...
plot_b <- ggplot(frog_data, aes(x = temp, y = freq)) +
  ...
  geom_smooth(method = "lm", se = F) +
  annotate("point", x = 21, y = 42.8, col = "orange", size = 3, pch = 4) +
  annotate("segment", x = 21, xend = 21, y = 36.37187, yend = 49.1473, col = "orange", alpha = 0.4) + # prediction interval
  geom_point() + 
...
x and y
  - Recall the formula: $b = r\frac{s_y}{s_x}$
- Can you describe the steps of a permutation test to test for a lack of correlation *directly*? 
