---
title: "Lecture 34: Analysis of Variance (ANOVA)"
subtitle: "Comparing means in more than two groups"
...
* Then, learn how to detect specifically which means are different
from one another using Tukey's HSD test in R

### Example A: Is there a difference between these means?

::: columns
:::: column
```{r, out.width="100%", echo=FALSE, warning=F, message=F}
library(ggplot2)
library(tidyr)
library(dplyr)
set.seed(1)
...
- The distribution of the data (black "X" marks) is wide enough that the distribution 
...
### Example B: Is there a difference between these means?
...
Try out this [Shiny App](https://shiny.abdn.ac.uk/Stats/apps/app_anova/#section-test-statistic).
...
* The MSG is like an average of the $k$ squared deviations, where groups with large
samples are up-weighted
...
between group means
* Go back to the Shiny App and move things around. See how the $F$-statistic changes. When is the $F$-statistic very high vs. 
...
- The interpretation of the $p$-value leaves something to be desired: **Which** 
...
in means is 20 and the 95% CI is -14.3 to 54.3. The adjusted $p$-value is 0.38.

- "Adjusted" means that it is adjusted for conducting multiple tests. The unadjusted
$p$-value would be smaller. The unadjusted $p$-value would be < 0.05 if
the 95% CI didn't include 0.
...
Even though these two CIs don't overlap with the null value, for the other four
...
- Can use `group_by()` and `summarize()` to calculate the sample standard deviations to see if 
...
twice as large as the smallest one. I.e., $\frac{s_{max}}{s_{min}} < 2$
